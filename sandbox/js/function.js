...
//沙盒控制台
var consoleBox = $('#沙盒控制台')
function sandbox() {
    consoleBox.css('display', 'block')
}

$('#沙盒控制台-開啟').on('click', function () {
  sandbox()
})

$('#沙盒控制台-關閉').on('click', function () {
  consoleBox.css('display', 'none')
})

function sandBoxDateOverride(date){
  switch (date) {
    case 31:
      sandBoxDateRefresh('https://raw.githubusercontent.com/kiang/pharmacies/948c501508365ebc2ac8b2f777d4157a5f92652e/json/points.json');
      break;
    case 315:
      sandBoxDateRefresh('https://raw.githubusercontent.com/kiang/pharmacies/01cdaab195ea4561ad032945ca52a461b93699b5/json/points.json');
      break;
    case 41:
      sandBoxDateRefresh('https://raw.githubusercontent.com/kiang/pharmacies/2b39fb33108264883e24e89c55f1f1d12d542baf/json/points.json');
      break;
    case 415:
      sandBoxDateRefresh('https://raw.githubusercontent.com/kiang/pharmacies/9e21660735d69fa41d3fd70002c8f4e4cec94e3d/json/points.json');
      break;
    case 423:
      sandBoxDateRefresh('https://raw.githubusercontent.com/kiang/pharmacies/8aeb1cdb8ebc0fe260b4c22e5d580f7fa1f54759/json/points.json');
      break;
    case 51:
      sandBoxDateRefresh('https://raw.githubusercontent.com/kiang/pharmacies/cebb8d03fd33abff4fad79c12b0f5261cb589c45/json/points.json');
      break;
    case 515:
      sandBoxDateRefresh('https://raw.githubusercontent.com/kiang/pharmacies/80adfb17d8f5073494e170876533680bb635f9ef/json/points.json');
      break;
  }
}
var OverriddedDateSource = 'https://raw.githubusercontent.com/kiang/pharmacies/master/json/points.json';
function sandBoxDateRefresh (url){
  OverriddedDateSource = url
  $('#地圖').remove()
  $('#地圖容器').append('<div id="地圖"></div>')
  init()
  loadData()
}

$(document).on('click', '.沙盒控制台-選項', function (e) {
  $('.沙盒控制台-選項').removeClass('啟用')
  $(this).addClass('啟用')
})
...
function loadData()
{
  ...
  popWindow('請稍候','','n');
  $.get(OverriddedDateSource, function(source)
  {
    ...
    {
      // s["features"][i].properties.mask_adult = Math.floor(Math.random() * 300) + 1;
    ...
    if(cty.replace("臺", "台") == k)
    {
      county[k].forEach(function(d)
      {
        ...
        if(dis.startsWith(d) || (cty.replace("臺", "台") === "台南市" && d === "中西區" && (dis.startsWith("西區") || dis.startsWith("中區"))))
        {
          ...
          // item.properties.mask_adult = Math.floor(Math.random() * 200) + 1
          // item.properties.mask_child = Math.floor(Math.random() * 200) + 1
